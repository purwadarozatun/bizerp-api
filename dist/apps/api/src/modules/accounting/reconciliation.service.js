...
let ReconciliationService = class ReconciliationService {
    ...
    computeStatus(documentAmount, journalAmount, hasDraft) {
        if (journalAmount === 0 && !hasDraft)
            return 'unreconciled';
        if (hasDraft && journalAmount === 0)
            return 'pending';
        if (Math.abs(documentAmount - journalAmount) < 0.01)
            return 'reconciled';
        return 'discrepancy';
    }
    async findAll(organizationId, filters) {
        ...
        const allDocIds = [
            ...invoices.map((i) => ({ id: i.id, type: 'INVOICE' })),
            ...bills.map((b) => ({ id: b.id, type: 'BILL' })),
        ];
    ...
    async getSummary(organizationId) {
        ...
        const totalJournaled = journalEntries.reduce((s, je) => {
            const debit = je.lines
                .filter((l) => l.type === 'DEBIT')
                .reduce((ls, l) => ls + Number(l.amount), 0);
            return s + debit;
        }, 0);
    ...
    async flagDiscrepancy(documentId, documentTypeHint, organizationId, note, userId, userRole) {
        if (!['accountant', 'admin', 'owner'].includes(userRole)) {
            throw new common_1.ForbiddenException('Only Accountant role can flag discrepancies');
        }
        ...
        if (documentTypeHint === 'INVOICE') {
            ...
            if (!invoice)
                throw new common_1.NotFoundException(`Invoice ${documentId} not found`);
            documentType = 'INVOICE';
        }
            ...
            if (!bill)
                throw new common_1.NotFoundException(`Bill ${documentId} not found`);
            documentType = 'BILL';
        }
        else {
    ...
    async getDocumentReconciliationStatus(documentId, documentType, organizationId) {
        // Get the document
